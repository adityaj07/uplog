...
import { transferOwnershipService } from "@/services/member/transferOwnership.service";
import type {
  ListMembersParamInput,
  TransferOwnershipParamInput,
  UpdateMemberInput,
...
export async function transferOwnership(c: Context<HonoContext>) {
  try {
    const sessionUser = c.var.sessionUser!;
    const sessionUserCompany = c.var.company!;
    const param = c.req.param() as unknown as TransferOwnershipParamInput;
